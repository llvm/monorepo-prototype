...
SDNodeInfoEmitter::emitTypeConstraints(raw_ostream &OS) const {
  ...
  for (const auto &[EnumName, Nodes] : TargetNodesByName) {
    ...
    // SequenceToOffsetTable reuses the storage if the sequence matches another
    // sequence's *suffix*. It is more likely that we have a matching *prefix*.
    // Reverse the order to increase the likelihood of storage reuse.
...
static void emitDesc(raw_ostream &OS, StringRef EnumName,
  ...
  assert(all_of(drop_begin(Nodes), [&](const SDNodeInfo *Other) {
    return haveCompatibleDescriptions(N, Other);
  }));
